...
public class UDPLibPD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (outputValue >= 20 && outputValue <=30)//agrando(outputValue == 3 )//
        {
            Debug.Log("up");
            pdPatch.SendFloat("proximity", 0.9f);          
          //  pdPatch.SendBang("VolumeUp");
            transform.localScale += new Vector3(outputValue * multi, outputValue * multi, outputValue * multi);
        }

        if (outputValue >= 2 && outputValue <= 10)//achico(outputValue >= 4)//
        {
            Debug.Log("down");
            pdPatch.SendFloat("proximity", 0.3f);
